                    ...
                    @foreach($settings->where('group', 'general') as $setting)
                        <div class="mb-3">
                            ...
                            @elseif($setting->type === 'file')
                                @if($setting->value)
                                    <div class="mb-2">
                                        <img src="{{ asset('storage/uploads/settings/' . $setting->value) }}" class="img-thumb" alt="{{ $setting->label }}">
                                    </div>
                                    <small class="text-muted d-block mb-1">Laisser vide pour garder l'image actuelle</small>
                                @endif
                                <input type="file" name="settings_file[{{ $setting->key }}]" id="setting_{{ $setting->key }}" class="form-control" accept="image/*">
